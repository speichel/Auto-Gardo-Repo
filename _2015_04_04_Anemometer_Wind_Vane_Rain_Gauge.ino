...
#define PIN_RAINGAUGE  3     // Digital 2
#define PIN_VANE        5     // Analog 5

// How often we want to calculate wind speed or direction
#define MSECS_CALC_WIND_SPEED 5000
#define MSECS_CALC_RAIN_GAUGE 5000
#define MSECS_CALC_WIND_DIR   5000

volatile int numRevsAnemometer = 0; // Incremented in the interrupt
volatile int numRevsRainGauge = 0; // Incremented in the interrupt

ulong nextCalcSpeed;                // When we next calc the wind speed
ulong nextCalcRainGauge;            // When we next calc the direction
...
void setup() {
   ...
   nextCalcRainGauge = millis() + MSECS_CALC_RAIN_GAUGE;
...
void loop() {
   ...
   if (time >= nextCalcRainGauge) {
      calcRainGauge();
      nextCalcRainGauge = time + MSECS_CALC_RAIN_GAUGE;
   } 
   if (time >= nextCalcDir) {
      calcWindDir();
...
//=======================================================
void countAnemometer() { numRevsAnemometer++; }
void countRainGauge() { numRevsRainGauge++; }
...
void calcRainGauge() {
   ...
   // (didn't calc right when done as one statement)
   long speed = 110;
   speed *= numRevsRainGauge;
   speed /= MSECS_CALC_RAIN_GAUGE;
   iDroplets = speed;         // Need this for formatting below
   ...
   numRevsRainGauge = 0;        // Reset counter
}
